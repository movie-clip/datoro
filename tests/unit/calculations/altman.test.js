 ...
 * @param {Object} financials - Company financials
 * @returns {number} - Z-Score (higher is better)
 */
function calculateAltmanZScore(financials) {
  ...
  const x4 = marketValueEquity / totalLiabilities // Market Cap / Total Liabilities
...
describe('Altman Z-Score Calculations', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should return NaN for undefined values (garbage in = garbage out)', () => {
      const financials = {
      ...
      expect(Number.isNaN(zScore)).toBe(true)
    })
  })
