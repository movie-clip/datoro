 ...
 * @returns {boolean} True if data is a configured series object
 * @example isConfiguredSeries({type: 'bar', name: 'Sales', data: [...]}) => true
 */
export function isConfiguredSeries(data) {
 ...
 * @param {*} data - Data to check
 * @returns {boolean} True if data is multi-series format
 * @example isMultiSeriesFormat([{name: 'FCF', data: [...]}, {name: 'SBC', data: [...]}]) => true
 */
export function isMultiSeriesFormat(data) {
  return (
    Array.isArray(data) &&
    data.length > 0 &&
 ...
 * @param {*} data - Data to check
 * @returns {boolean} True if data is simple time-series format
 * @example isSimpleTimeSeries([[1609459200000, 100], [1612137600000, 150]]) => true
 */
export function isSimpleTimeSeries(data) {
  return (
    Array.isArray(data) &&
    data.length > 0 &&
 ...
 * @param {*} data - Data to check
 * @returns {boolean} True if array contains configured series
 * @example isConfiguredSeriesArray([{type: 'bar', name: 'A', data: [...]}]) => true
 */
export function isConfiguredSeriesArray(data) {
  return isMultiSeriesFormat(data) && isConfiguredSeries(data[0])
}

/**
 * Check if data needs series configuration
 * @param {*} data - Data to check
 * @returns {boolean} True if data needs configuration (not already configured)
 */
export function needsSeriesConfiguration(data) {
  if (isConfiguredSeries(data)) {
    return false
  }
  if (isMultiSeriesFormat(data) && isConfiguredSeries(data[0])) {
    return false
  }
  return true
}
