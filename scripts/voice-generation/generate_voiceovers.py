...
USE_VOICE_CLONING = False  # Set to False to use preset voices
VOICE_NAME = "MyCustomVoice"  # Name for your cloned voice
...
PRESET_VOICE_ID = "tnSpp4vdxKPjI9w0GnoV"
    ...
    try:
        ...
        if not VOICE_SAMPLES_DIR.exists():
            raise FileNotFoundError(
                ...
                f"Please create this folder and add your voice samples (.mp3, .wav files)"
            )
        
        # Collect audio samples
        sample_files = list(VOICE_SAMPLES_DIR.glob("*.mp3")) + list(VOICE_SAMPLES_DIR.glob("*.wav"))
        if not sample_files:
            raise FileNotFoundError(
                f"No audio samples found in {VOICE_SAMPLES_DIR}\n"
                f"Please add .mp3 or .wav files of your voice (at least 1 minute total)"
            )
        ...
        voice = client.voices.add(
            name=voice_name,
            description=f"Custom cloned voice created from samples",
            files=[str(f) for f in sample_files[:25]],  # Max 25 samples
        )
