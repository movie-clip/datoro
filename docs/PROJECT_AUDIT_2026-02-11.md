...
4. Reduce logging volume in hot paths (batch endpoint logs can be very chatty in prod).
...
- Make rate limiting cluster-safe (Redis store + Redis global quota limiter).
- Add CSRF defense (or change cookie policy so CSRF is not applicable).

### Phase 2 (1–2 days)

- Fix batch caching strategy for dynamic vs static data.
- Add request coalescing to `/api/ticker-data`.
- Remove the 500ms batch delay.

### Phase 3 (cleanup)

- Tighten CSP in production.
- Replace `$executeRawUnsafe`.
- Consolidate validation libraries (Joi vs Zod vs express-validator) to reduce complexity.
